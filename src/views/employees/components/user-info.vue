...
export default {
  ...
  methods: {
    async getUserId () {
      this.userInfo = await getUserId(this.userId)
    ...
    async getPersonalDetail () {
      ...
      if (this.formData.staffPhoto && this.formData.staffPhoto.trim()) {
        this.$refs.myStaffPhoto = [{ url: this.formData.myStaffPhoto, upload: true }]
      }
    },
    async saveUser () {
      const fileList = this.$refs.staffPhoto.fileList
      if (fileList.some(item => !item.upload)) {
        return this.$message.warning('tupian 未上传完成')
      }

      await saveUserDetailById({ ...this.userInfo, staffPhoto: fileList.length ? fileList[0].url : ' ' })
      this.$message.success('保存基本信息')
    },
    async savePersonal () {
